...
| `--encore-hole-r` | `max(70, plateW × 0.42 × crowdBoost)` | `plateW = 1500 × layout.scale`. `crowdBoost` is 1 at n≤5 (Handhelds), 1.25 at n≥15 (Munchies), lerp between. 160 is only a CSS fallback. Preview uses this same formula. New stickers live on a **second lattice** (`.family-portrait-sticker-rig`) above the veil; rig opacity follows `--motion-veil` (same clock as the Spotlight Veil). Sticker scale is `photoScale × 0.9 × crowdBoost × rowBoost` — rowBoost 1.5 at ≤2 rows (Handhelds +50%), 1.75 at ≥4 rows (Munchies +75%). Right-edge badges that would clip the stage sit on the top-left of the hole. |
