...
| `#/menu/board/1/item/new` … `/item/0` | Create Item / Edit Item (Inventory fields + photo) on the selected catalog, including Restaurant Copy |
| `#/menu/board/1/item/0/image` | Edit Image (`.menuimg` 1500×1000 guide + sliders). Saved Plate Image is hosted so TVs can load it without a git push |
...
- **Standalone** [`uploader.html`](../uploader.html) remains for Suite / operator use. Menu Manager hosts the same write (`POST /api/manager/item`) as Edit Item / Create Item on boards 1–3 (Restaurant Copy included). A hosted Plate Image URL is written to the Image cell so live TVs can load it without a git push. Footer boxes (Proteins / Sauces / Drinks / Veggies) still use the Suite uploader.
