...
Phone hub: [`suite.html`](../suite.html) — Suite, Deployer, Tickets, Menu Manager, boards. The top bar is the same on Suite, Deployer, Tickets, Menu Manager splash, and Listener home (current page is plain text in that slot). Shared list: `js/surfaces.js` `nav`.
