...
**Last updated:** 2026-08-21 02:20 (Announcement color HTML)  
...
- **Announcement Text:** markdown (cheat-sheet basics + tables / strike / task lists / highlight) plus typed color HTML (`<font color>`, `<mark style="background">`). Plain CSV; xlsx rich runs stay quarantined. Alt+Enter = new line on the board. Type is hyphenated and scaled so it never leaves the Announcement Body. Authoring cheat sheet + demo examples: [ANNOUNCEMENT_MARKDOWN.md](./ANNOUNCEMENT_MARKDOWN.md).
