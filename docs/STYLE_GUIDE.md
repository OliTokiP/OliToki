...
- Flex wrap with **balanced rows** (character/pixel budgeting so rows look even — reference `screenshots/footer-verify.png` / sauces packing). After the pack is chosen, rows stack **fullest on top** on every wrap-mode Footer Box (Proteins, Sauces, Drinks, Veggies). Fullest is **painted width**: bold name + parenthetical subtitle at 0.82em Regular + Regular price — not character count at body size. Shared packer: `js/box-pack.js`. Each packed line is a nowrap `.wrap-line-row` so a planned row never splits into a visual extra line.
